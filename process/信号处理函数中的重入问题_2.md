...
void handler(int sig) {
    ...
    for (int i = 0; i < 10; i++) {
        ...
        sleep(1);
        // 如果在这期间按Ctrl+C，不会中断这个循环，而是把信号保留下来，设置为pending状态
        // 直到handler完成后，才会处理这个pending信号（且多个信号在阻塞期间只设置一次pending状态）
    }
...
=== handler完成，回到main() ===
=== 进入handler，中断了main() ===
当前在handler中，PID=41177
handler工作中...0
```

看到了吗？第二次Ctrl+C无法中断handler，而是把信号保留下来，多个信号算是一次
...
void FathEXIT(int sig) {
    ...
    // 现在安全了，即使有不同类信号也不会中断当前函数，有同类信号也不会再次执行
