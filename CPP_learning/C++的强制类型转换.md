...
## 延展  
它们是 C++ 语言为了提供更安全、更明确的类型转换，而引入的“具名转换操作符”。C 语言中那种简单的 (type)value 写法（被称为 C-style cast）过于模糊和强大，容易出错，所以在 C++ 中推荐使用这几种更具体的 cast：

static_cast：用于相关类型间的转换（如数字类型转换、类继承体系中的上下行转换）。
reinterpret_cast：用于不相关类型间的底层位模式重解释，主要是指针类型。
dynamic_cast：专门用于类继承体系中的下行转换，带有运行时类型检查，是最安全的类指针转换。
const_cast：用于添加或移除变量的 const 或 volatile 属性。
你会在任何标准的 C++ 项目中看到它们，与是否使用 Qt 框架无关。